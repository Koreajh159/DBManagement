...
public class TableAlterDialog extends JDialog{
	JPanel p_main, p_bt, p_bt1, p_bt2, p_mainLabel, p_mainForm;
	JScrollPane scroll;
	JScrollBar bar;
	JButton bt_update, bt_cancel, bt_add, bt_del;
	...
	String table_name;
	MainFrame main;
	public TableAlterDialog(MainFrame main, String table_name) {
		...
		bt_update.setMargin(new Insets(0,0,0,0));
		bt_update.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent e) {
				SetOracleQuery.alterTable(main.conn, table_name);
			}
		});
	...
	public void initSetting() {
		String[][] descInfo = SetOracleQuery.descTable(main.conn, table_name);
		for(int i=0; i<descInfo.length;i++) {
			createInputForm();
			JPanel p = arr_p.get(i);
			JTextField t_colname = (JTextField)p.getComponent(0);
			Choice ch_dataType = (Choice)p.getComponent(1);
			JTextField t_size = (JTextField)p.getComponent(2);
			Choice ch_limit = (Choice)p.getComponent(4);
			t_colname.setText(descInfo[i][0]);
			ch_dataType.select(descInfo[i][1].toLowerCase());
			t_size.setText(descInfo[i][2]);
			ch_limit.select(descInfo[i][3]);
		}
	}
